...
#define XCELL(j,k) VEC2(xcell,j,k,ndim)
  ...
  // Vector de enteros con signo de tamaño igual al número de caras totales en toda la frontera, que contendrá útilmente solo a los ids de las caras en la frontera deseada (el resto del vector quedará con valores basura) (desde gts_soft_2, se usan 4 patchs para el GTS)
  vector<cs_lnum_t> face_list(nbfaces);
  // Se guardará el número de caras sobre la frontera deseada (desde gts_soft_2, se usan 4 patchs para el GTS)
  cs_lnum_t nfcext1, nfcext2, nfcext3, nfcext4, nfcext;
  ...
  cs_selector_get_b_face_list("GTSr_f_r",&nfcext1,face_list.data()); // gtsModel
  cs_selector_get_b_face_list("GTSrear",&nfcext2,&face_list[nfcext1]);
  cs_selector_get_b_face_list("GTSright",&nfcext3,&face_list[nfcext1+nfcext2]);
  cs_selector_get_b_face_list("GTSleft",&nfcext4,&face_list[nfcext1+nfcext2+nfcext3]);
  nfcext = nfcext1 + nfcext2 + nfcext3 + nfcext4;
  ...
  // Cálculo de coeficientes de fuerzas (arrastre, fuerza lateral y sustentacion)
  if (cs_glob_rank_id<=0)
  {
    // Coeficiente de arrastre
    cs_real_t c_drag, c_lat, c_z;
    // Densidad  
    cs_fluid_properties_t *fp = cs_get_glob_fluid_properties();
    cs_real_t density_fluid = fp->ro0;
    // Velocidad de referencia considerada en modelo de turbulencia (es la velocidad a la entrada) // Se podria usar un dato ingresado en el apartado de notebook
    ...
    c_lat = force[1]/(density_fluid*u_ref*u_ref*frontal_area/2);
    c_z = force[2]/(density_fluid*u_ref*u_ref*frontal_area/2);
    printf("Drag coefficient: %f \n", c_drag);
    printf("Lateral coefficient: %f \n", c_lat);
    printf("Lift coefficient: %f \n", c_z);
  }
