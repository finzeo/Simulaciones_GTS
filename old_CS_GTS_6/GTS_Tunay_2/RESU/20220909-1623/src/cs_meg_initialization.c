                      ...
                      const cs_zone_t *vz)
{
  ...
  if (strcmp(vz->name, "all_cells") == 0 &&
      strcmp(field_name, "velocity") == 0 ) {

    const int vals_size = vz->n_elts * 3;
    BFT_MALLOC(new_vals, vals_size, cs_real_t);


    for (cs_lnum_t e_id = 0; e_id < vz->n_elts; e_id++) {
      cs_lnum_t c_id = vz->elt_ids[e_id];

      new_vals[3 * e_id + 0] =  1.2264 ;
      new_vals[3 * e_id + 1] =  0. ;
      new_vals[3 * e_id + 2] =  0. ;
    }
  }
