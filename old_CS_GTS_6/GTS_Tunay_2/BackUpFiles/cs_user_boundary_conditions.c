...
/////    if ((BFCOG(fid,0)>=-7.5*h && BFCOG(fid,0)<=7.99*h) && (BFCOG(fid,1)<=3.0*h && BFCOG(fid,1)>=-3.0*h) && (BFCOG(fid,2)>=-0.01*h && BFCOG(fid,2)<=0.01*h))
...
/////   rcodcl[iscal*nfaces + fid] = ux_inf;
    if ( (BFCOG(fid,2)>=-0.001*h && BFCOG(fid,2)<=0.001*h) && ((BFCOG(fid,0)<-7.99*h || BFCOG(fid,0)>7.5*h) || (BFCOG(fid,1)>3.0*h || BFCOG(fid,1)<-3.0*h)) )
