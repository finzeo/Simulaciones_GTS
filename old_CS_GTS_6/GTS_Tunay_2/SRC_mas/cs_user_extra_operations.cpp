...
#define BFORCES(fid,k) bforces[3*fid+k]
  ...
  if (do_fsi) {
    ...
    cs_real_t *disale = cs_field_by_name("disale")->val;
    cs_real_t *disale2 = cs_field_by_name("mesh_disp_rel")->val;
#define DISALE(v,k) disale[3*v+k]
#define DISALE2(v,k) disale2[3*v+k]
    for (int j=0; j<nvrtx; j++) {
      DISALE2(j,0) = DISALE(j,0);
      DISALE2(j,1) = DISALE(j,1);
      DISALE2(j,2) = DISALE(j,2)-dzbox;
    }
  }

  GETDD(rho1);
  GETDD(rho2);
  // Apparently there are two possibilities for computing
  // the mass.  One is in basis to the density field
  // (rhomass), the other in the void_fraction field
  // (vfmass).

  static cs_real_t vfmass0=NAN,cmass=NAN;
  cs_real_t vfmass=0.0;
  cs_real_t *vfvals = cs_field_by_name("void_fraction")->val;
  for (cs_lnum_t j=0; j<ncells; j++) vfmass += vfvals[j]*cell_vol[j];
  cs_parall_sum(1,CS_DOUBLE,&vfmass);
  if (ISNAN(vfmass0)) {
    vfmass0=vfmass;
    GETD(cmass);
    PPRINTF("Using cmass %g\n",cmass);
  }

  PPRINTF("vfmass0 %g, vfmass %g\n",vfmass0,vfmass);
  // We add a control term 
  for (cs_lnum_t j=0; j<ncells; j++) {
    double &vf = vfvals[j];
    vf -= cmass*(vfmass-vfmass0);
    // Cutoff the values to [0,1]
    if (vf<0.0) vf=0.0;
    else if (vf>1.0) vf=1.0;
  }
}
